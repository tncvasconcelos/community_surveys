...
# Clean subset for modeling (removes rows with NAs in key columns)
subset_bees <- subset_bees[complete.cases(subset_bees[, c("longitude", "latitude", "bee", "prop_bee_angios", predictors_discrete)]), ]
subset_biome_comparison <- subset(subset_bees, biome %in% names(table(subset_bees$biome))[table(subset_bees$biome) > 2])
